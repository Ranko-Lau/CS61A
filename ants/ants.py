...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        p = self.place
        i = 0
        while (i < self.lower_bound and p.entrance != None):
            p = p.entrance
            i += 1
        if p.entrance == None:
            return None
        while (p.bees == [] and not(p.is_hive) and i < self.upper_bound):
            p = p.entrance
            i += 1
        if p.is_hive:
            return None
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown > 0:
            ...
            att = random_bee(self.place.bees)
            att.reduce_health(att.health)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while p.exit != None:
            ...
            if p.ant == None:
                continue
            elif not p.ant.is_container and not p.ant.doubled:
                p.ant.double()
            elif p.ant.is_container:
                if not p.ant.doubled: p.ant.double()
                if p.ant.ant_contained != None and not p.ant.ant_contained.doubled: p.ant.ant_contained.double()
        # END Problem 12
