...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        sym = signature.first
        arg = signature.rest
        env.define(sym, do_lambda_form(Pair(arg, expressions.rest), env))
        return sym
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions is nil: return True
        ...
        while current.rest != nil:
            if is_scheme_false(scheme_eval(current.first, env)): return False
            else: return do_and_form(current.rest, env)
...
def do_or_form(expressions, env):
    ...
    if expressions is nil: return False
        ...
        current = expressions
        while current.rest != nil:
            if is_scheme_true(scheme_eval(current.first, env)): return scheme_eval(current.first, env)
            else: return do_or_form(current.rest, env)
        return scheme_eval(current.first, env)
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            if clause.rest is not nil: return eval_all(clause.rest, env)
            else: return test
            # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    while current is not nil:
        ...
        validate_form(current.first.rest, 1, 1)
        vals = Pair(scheme_eval(current.first.rest.first, env), vals)
